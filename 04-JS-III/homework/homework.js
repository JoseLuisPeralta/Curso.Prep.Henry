...
function incrementarPorUno(arr) {
  // "arr" debe ser una matriz de enteros (int/integers)
...
function agregarNumeros(numbers) {
  ...
  for(let i of numbers){
    suma+=i; //que es lo mismo suma = suma + i;
  }
  return suma;
}
...
function diaDeLaSemana(numeroDeDia) {
  ...
  switch(numeroDeDia){
    ...
    break;
    default : return "Los dias de la semana son 7"
    break;
 }
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí
  let comienzo = arreglo[0]; 
  let valor = null;
  for(let i = 0; i < arreglo.length; i++){
    if(comienzo === arreglo[i]){
      valor +=1;
    }
  }
...
function breakStatement(numero) {
  ...
  while(num <= 10){
    numero+=2;  
    switch(numero){
      case 10 : return "Se interrumpió la ejecución";
      break
    }
    miArray.push(numero);
    num ++;
